...
if __name__ == '__main__':
	...
	for param in ['mode_spatial', 'mode_cumulative', 'mode_model_bias', 'N_observe', 'P_c', 'P_copyFail', 'N_migrate', 'r_max']:
		sim_parameters[param] = getattr(args, param)
	...
	coll_generations = db['gp_' + args.d]
	...
	if last_run:
		...
		GP = solegene.Generation(sim_parameters=sim_parameters, use_cloud=args.cloud, empty=True)
		...
		GP = solegene.Generation(sim_parameters=sim_parameters, use_cloud=args.cloud)

#	for i in xrange(start_generation, args.n):
	if start_generation < args.n:
		i = start_generation
		logger.debug("======================================================================")
