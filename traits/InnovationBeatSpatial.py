...
class InnovationBeatSpatial(Trait):
    ...
    identify its cycle for the rest of this state.
    ...
    def done(self, entryRound,
        ...
        # Don't be tempted to use currentDeme -- remember, this needs to be traceable in the future too, not just on
        # the round where the change occurred!
        idx_entryRound = historyRounds.index(entryRound)
        if (roundsAlive <= entryRound):
            # Give us a chance first!
            return False
        else:
            initial_deme = historyDemes[idx_entryRound+1]
            result = False
            for (change_round, deme) in zip(historyRounds[idx_entryRound+1:],historyDemes[idx_entryRound+1:]):
                if deme != initial_deme:
                    result = True
                    break
            if result == True:
                #print ("Entered at round %d, changed at round %d. Initial deme %d, final deme %d." 
                #    % (entryRound, change_round, initial_deme, deme))
